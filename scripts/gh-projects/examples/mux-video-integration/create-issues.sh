...
read -r P1_C1_URL P1_C1_NUM _ <<<"$(create_child "Install @mux/mux-player-astro dependency" \
  "$F" "mux,phase-1" "$P1_NUM")"
echo "  C1: $P1_C1_URL"

F=$(prep_body "$SCRIPT_DIR/phase-1/c2.md" "$P1_NUM")
read -r P1_C2_URL P1_C2_NUM _ <<<"$(create_child "Extend projects schema with optional muxPlaybackId" \
  "$F" "mux,phase-1" "$P1_NUM")"
echo "  C2: $P1_C2_URL"

F=$(prep_body "$SCRIPT_DIR/phase-1/c3.md" "$P1_NUM")
read -r P1_C3_URL P1_C3_NUM _ <<<"$(create_child "Wire MuxPlayer into ProjectLayout + CSS + swipe-watch frontmatter" \
  "$F" "mux,phase-1" "$P1_NUM")"
echo "  C3: $P1_C3_URL"

# C4 references C3 via __C3_NUM__
F=$(prep_body "$SCRIPT_DIR/phase-1/c4.md" "$P1_NUM" "" "" "$P1_C3_NUM")
read -r P1_C4_URL P1_C4_NUM _ <<<"$(create_child "Phase 1 manual QA: player, fallback, OG, screenshot" \
  "$F" "mux,phase-1" "$P1_NUM")"
echo "  C4: $P1_C4_URL"
...
read -r P2_C1_URL P2_C1_NUM _ <<<"$(create_child "Add .env.example for PUBLIC_MUX_ENV_KEY" \
...
read -r P2_C3_URL P2_C3_NUM _ <<<"$(create_child "Pass env-key and metadata to MuxPlayer" \
  "$F" "mux,phase-2" "$P2_NUM")"
echo "  C3: $P2_C3_URL"

F=$(prep_body "$SCRIPT_DIR/phase-2/c2.md" "$P2_NUM" "" "" "$P2_C3_NUM")
read -r P2_C2_URL P2_C2_NUM _ <<<"$(create_child "Provision PUBLIC_MUX_ENV_KEY in 1Password + Firebase" \
  "$F" "mux,phase-2" "$P2_NUM")"
echo "  C2: $P2_C2_URL"

F=$(prep_body "$SCRIPT_DIR/phase-2/c4.md" "$P2_NUM")
read -r P2_C4_URL P2_C4_NUM _ <<<"$(create_child "Phase 2 manual QA: confirm Mux Data views" \
  "$F" "mux,phase-2" "$P2_NUM")"
echo "  C4: $P2_C4_URL"
...
read -r P3_C1_URL P3_C1_NUM _ <<<"$(create_child "Extract ProjectMuxPlayer.astro component" \
  "$F" "mux,phase-3" "$P3_NUM")"
echo "  C1: $P3_C1_URL"

# C2 references C1
F=$(prep_body "$SCRIPT_DIR/phase-3/c2.md" "$P3_NUM" "$P3_C1_NUM")
read -r P3_C2_URL P3_C2_NUM _ <<<"$(create_child "Swap ProjectLayout to call ProjectMuxPlayer" \
  "$F" "mux,phase-3" "$P3_NUM")"
echo "  C2: $P3_C2_URL"

F=$(prep_body "$SCRIPT_DIR/phase-3/c3.md" "$P3_NUM")
read -r P3_C3_URL P3_C3_NUM _ <<<"$(create_child "Document MUX video extension path in AGENTS.md" \
  "$F" "mux,phase-3" "$P3_NUM")"
echo "  C3: $P3_C3_URL"

F=$(prep_body "$SCRIPT_DIR/phase-3/c4.md" "$P3_NUM")
read -r P3_C4_URL P3_C4_NUM _ <<<"$(create_child "Phase 3 theming QA: verify all accent classes" \
  "$F" "mux,phase-3" "$P3_NUM")"
echo "  C4: $P3_C4_URL"
...
read -r P4_C1_URL P4_C1_NUM _ <<<"$(create_child "Add scripts/refresh-mux-gifs.mjs" \
  "$F" "mux,phase-4" "$P4_NUM")"
echo "  C1: $P4_C1_URL"

F=$(prep_body "$SCRIPT_DIR/phase-4/c2.md" "$P4_NUM")
read -r P4_C2_URL P4_C2_NUM _ <<<"$(create_child "Skip Mux-backed projects in refresh-social-preview-images.mjs" \
  "$F" "mux,phase-4" "$P4_NUM")"
echo "  C2: $P4_C2_URL"

# C3 references C1 and C2
F=$(prep_body "$SCRIPT_DIR/phase-4/c3.md" "$P4_NUM" "$P4_C1_NUM" "$P4_C2_NUM")
read -r P4_C3_URL P4_C3_NUM _ <<<"$(create_child "Wire refresh-mux-gifs.mjs into build" \
  "$F" "mux,phase-4" "$P4_NUM")"
echo "  C3: $P4_C3_URL"

# C4 references C3
F=$(prep_body "$SCRIPT_DIR/phase-4/c4.md" "$P4_NUM" "" "" "$P4_C3_NUM")
read -r P4_C4_URL P4_C4_NUM _ <<<"$(create_child "Document Mux GIF refresher in AGENTS.md" \
  "$F" "mux,phase-4" "$P4_NUM")"
echo "  C4: $P4_C4_URL"

F=$(prep_body "$SCRIPT_DIR/phase-4/c5.md" "$P4_NUM")
read -r P4_C5_URL P4_C5_NUM _ <<<"$(create_child "Phase 4 manual QA: GIF regeneration + hero refresher skip" \
  "$F" "mux,phase-4" "$P4_NUM")"
echo "  C5: $P4_C5_URL"
